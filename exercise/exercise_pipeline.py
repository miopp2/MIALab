...
def collect_image_paths(data_dir):
    ...
    # create an instance of futil.FileSystemDataCrawler and pass the corresponding arguments
    crawler_ = futil.FileSystemDataCrawler(f'{os.path.dirname(os.path.realpath(__file__))}/{data_dir}', image_keys,
                                           MyFilePathGenerator)
...
def load_images(image_paths):
    ...
    image_dict = {
        structure.BrainImageTypes.T1w: sitk.ReadImage(list(image_paths.values())[0], outputPixelType=sitk.sitkFloat32),
        structure.BrainImageTypes.GroundTruth: sitk.ReadImage(list(image_paths.values())[1],
...
def register_images(image_dict, atlas_img):
    ...
    # apply transform to GroundTruth image (gt_img) (hint: sitk.Resample, referenceImage=atlas_img,
